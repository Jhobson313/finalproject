		...
		<table>
			...
			<c:forEach var="deposit" items="${deposits}">
				<tr>
					...
					<td><c:out value="${ deposit.depositAmount }" /> <a
						href="<c:url value="/deposits?goal=${deposit.goalId}"/>">(filter)</a></td>

					<td><c:out value="${ deposit.currentDate }" /> <a
						href="<c:url value="/deposits?date=${deposit.currentDate}"/>">(filter)</a></td>
